...
from structures import IceCandidate, CandidatesCollection
...
class ClientLogParser:
    ...
    ICE_SIMPLE_PATTERN = re.compile(r"ICE message for connection '(\d+)/(\d+)'")

    DISCONNECT_PATTERN = re.compile(r"Message from game: 'Disconnected' '\[(.*?)\]'")
    ...
    def parse(self) -> CandidatesCollection:
        ...
        print(
            f"DEBUG: Первая проблемная строка: {self.debug_lines[:1] if self.debug_lines else 'OK'}"
        )
    ...
    def _apply_disconnect_times(self):
        for candidate in self.candidates:
            if candidate.player_uid in self.disconnect_events:
                candidate.time_disconnected = self.disconnect_events[
