...
class Config:
    ...
    def _load_env(self):
        ...
        db_file_str = os.getenv("FAF_DB_FILE", self._get("database.db_file", "faf_analysis.db"))
        ...
        if not self.logs_dir.exists():
            ...
            print(f"   Проверьте путь в faf_config.toml")
        else:
            print(f"✅ Директория логов найдена: {self.logs_dir}")
        
        self.auto_mark_ddos = os.getenv("FAF_AUTO_MARK_DDOS", str(self._get("analysis.auto_mark_ddos", False))).lower() == "true"
        self.ip_threshold = float(os.getenv("FAF_IP_THRESHOLD", str(self._get("analysis.ip_threshold", 0.5))))
