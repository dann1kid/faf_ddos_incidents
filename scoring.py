...
def recompute_risk_scores(now=None):
    ...
    with db.atomic():
        for p in Player.select():
            ...
            ddos_sessions = (PlayerSession
                             .select(PlayerSession, Match)
                             .join(Match)
                             .where((PlayerSession.player == p) & (Match.ddos_detected == True)))
            ...
            incidents = (DDoSIncident
                         .select()
                         .where(DDoSIncident.target_ip.is_null(False)))
            ...
            ddos_count = (PlayerSession
                          .select()
                          .join(Match)
                          .where((PlayerSession.player == p) &
                                 (Match.ddos_detected == True) &
                                 ((PlayerSession.joined_at >= day_ago) | (PlayerSession.left_at >= day_ago))))
