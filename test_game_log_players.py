...
def test_game_log_parser():
    ...
    if not log_path.exists():
        ...
        print("Пожалуйста, помести game_*.log в ту же директорию или укажи правильный путь")
        return
    ...
    for uid, player in data['players'].items():
        ...
        print(f"{marker} UID: {uid:8} | Nick: {player['nick']:20} | Role: {player.get('role', 'unknown')}")
    
    print("-" * 60)
    print("⏱️  Сессии (время входа/выхода):")
    
    # Сортировка по времени входа
    sessions_sorted = sorted(data['sessions'], key=lambda x: x['joined_at'] or datetime.min)
    
    for session in sessions_sorted:
        join_time = session['joined_at'].strftime('%H:%M:%S') if session['joined_at'] else "Unknown"
        leave_time = session['left_at'].strftime('%H:%M:%S') if session['left_at'] else "Still connected"
        
        print(f"   {session['player_nick']:20} | In: {join_time} | Out: {leave_time} | Role: {session['role']}")
